...
function isStarterBuilding(
  x: number,
  y: number,
  buildingType: string,
): boolean {
  if (buildingType === "house_small" || buildingType === "shop_small")
    return true;
...
function perlinNoise(
  ...
  for (let i = 0; i < octaves; i++) {
    total +=
      interpolatedNoise(x * frequency, y * frequency, seed + i * 100) *
      amplitude;
    maxValue += amplitude;
    amplitude *= 0.5;
    frequency *= 2;
...
function generateLakes(
  ...
  if (lakeCenters.length === 0) {
    ...
    const threeQuarterSize = Math.min(
      size - safeZone,
      Math.floor((size * 3) / 4),
    );
    lakeCenters.push(
  ...
  const selectedCenters = lakeCenters.slice(
    0,
    Math.min(numLakes, lakeCenters.length),
  );
...
function generateOceans(
  ...
  const coastNoise = (x: number, y: number) =>
    perlinNoise(x, y, seed + 2000, 3);
  ...
  const generateOceanEdge = (
    ...
    for (let i = spanStart; i < spanEnd; i++) {
      ...
      const localDepth = Math.max(
        1,
        Math.min(Math.floor(rawDepth * edgeFade), maxDepth),
      );
  ...
  for (const edge of edges) {
    if (edge.tiles.length > 0) {
      const avgX =
        edge.tiles.reduce((sum, t) => sum + t.x, 0) / edge.tiles.length;
      const avgY =
        edge.tiles.reduce((sum, t) => sum + t.y, 0) / edge.tiles.length;
...
export function getConnectableCities(
  ...
  for (const city of adjacentCities) {
    if (
      city.discovered &&
      !city.connected &&
      hasRoadAtEdge(grid, gridSize, city.direction)
    ) {
...
function generateTerrain(size: number): {
  grid: Tile[][];
...
const WATERFRONT_BUILDINGS: BuildingType[] = [
  "marina_docks_small",
  "pier_large",
];
...
export function getWaterAdjacency(
  ...
  for (let dx = 0; dx < width; dx++) {
    ...
    if (
      checkY < gridSize &&
      grid[checkY]?.[checkX]?.building.type === "water"
    ) {
...
export function getRoadAdjacency(
  ...
  if (!roadOnSouthOrEast) {
    for (let dy = 0; dy < height; dy++) {
      ...
      if (
        checkX < gridSize &&
        (checkType === "road" || checkType === "bridge")
      ) {
        roadOnSouthOrEast = true;
        break;
      }
...
function detectBridgeOpportunity(
  ...
  const northOpp = scanForBridgeInDirection(
    grid,
    gridSize,
    x,
    y,
  ...
  const southOpp = scanForBridgeInDirection(
    grid,
    gridSize,
    x,
    y,
  ...
  // East (x stays, y-1)
  const eastOpp = scanForBridgeInDirection(
    grid,
    gridSize,
    x,
    y,
  ...
  // West (x stays, y+1)
  const westOpp = scanForBridgeInDirection(
    grid,
    gridSize,
    x,
    y,
...
function checkAndCreateBridges(
  ...
  const opportunity = detectBridgeOpportunity(
    grid,
    gridSize,
    placedX,
    placedY,
...
function calculateServiceCoverage(
  ...
  for (const building of serviceBuildings) {
    ...
    if (type === "power_plant") {
      ...
      const serviceType = (
        config as { type: "police" | "fire" | "health" | "education" }
      ).type;
      ...
      for (let ny = minY; ny <= maxY; ny++) {
        for (let nx = minX; nx <= maxX; nx++) {
          ...
          if (distSquared <= rangeSquared) {
            ...
            currentCoverage[ny][nx] = Math.min(
              100,
              currentCoverage[ny][nx] + coverage,
            );
          }
        }
      }
...
function hasRoadAccess(
  ...
  while (queueHead < queueTail) {
    ...
    for (const [nx, ny] of neighbors) {
      ...
      if (
        neighbor.building.type === "road" ||
        neighbor.building.type === "bridge"
      ) {
        return true;
      }

      const isPassableZone =
        neighbor.zone === startZone && neighbor.building.type !== "water";
...
function evolveBuilding(
  ...
  if (
    ...
    building.constructionProgress = Math.min(
      100,
      building.constructionProgress + constructionSpeed,
    );
          ...
          for (let dy = 0; dy < size.height; dy++) {
            for (let dx = 0; dx < size.width; dx++) {
              ...
              if (clearTile) {
                ...
                clearedBuilding.powered =
                  services.power[y + dy]?.[x + dx] ?? false;
                clearedBuilding.watered =
                  services.water[y + dy]?.[x + dx] ?? false;
  ...
  if (zoneDemandValue < -20 && building.age > 30) {
    ...
    const abandonmentChance = Math.min(
      0.02,
      Math.abs(zoneDemandValue + 20) / 4000,
    );

    // Buildings without power/water are slightly more likely to be abandoned (except starter buildings)
    const utilityPenalty = isStarter
      ? 0
      : (!hasPower ? 0.005 : 0) + (!hasWater ? 0.005 : 0);
  ...
  if (
    hasUtilitiesForConsolidation &&
    building.age > ageRequirement &&
  ...
  anchorBuilding.population =
    buildingStats?.maxPop > 0
      ? Math.floor(
          buildingStats.maxPop *
            Math.max(1, anchorBuilding.level) *
            efficiency *
            0.8,
        )
      : 0;
  anchorBuilding.jobs =
    buildingStats?.maxJobs > 0
      ? Math.floor(
          buildingStats.maxJobs *
            Math.max(1, anchorBuilding.level) *
            efficiency *
            0.8,
        )
...
function calculateStats(
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      if (building.type === "park" || building.type === "park_large")
        parkCount++;
  ...
  const baseCommercialDemand =
    (population * 0.3 - jobs * 0.3) / 4 + subwayBonus;
  ...
  const residentialDemand = Math.min(
    ...
    Math.max(
      -100,
      residentialWithBonuses * taxMultiplier + taxAdditiveModifier,
    ),
  );
  const commercialDemand = Math.min(
    ...
    Math.max(
      -100,
      commercialWithBonuses * taxMultiplier + taxAdditiveModifier * 0.8,
    ),
  );
  const industrialDemand = Math.min(
    ...
    Math.max(
      -100,
      industrialWithBonuses * taxMultiplier + taxAdditiveModifier * 0.5,
    ),
  );
  ...
  expenses += Math.floor(
    (budget.education.cost * budget.education.funding) / 100,
  );
  expenses += Math.floor(
  ...
  const environment = Math.min(
    100,
    Math.max(0, greenRatio * 200 - pollutionRatio * 100 + 50),
  );

  const jobSatisfaction =
    jobs >= population ? 100 : (jobs / (population || 1)) * 100;
...
function updateBudgetCosts(grid: Tile[][], budget: Budget): Budget {
  ...
  newBudget.transportation.cost =
    roadCount * 2 + subwayTileCount * 3 + subwayStationCount * 25;
...
function generateAdvisorMessages(
  ...
  if (abandonedBuildings > 0) {
    ...
    if (abandonedResidential > 0)
      details.push(`${abandonedResidential} residential`);
    if (abandonedCommercial > 0)
      details.push(`${abandonedCommercial} commercial`);
    if (abandonedIndustrial > 0)
      details.push(`${abandonedIndustrial} industrial`);
    ...
    messages.push({
      ...
      priority:
        abandonedBuildings > 10
          ? "high"
          : abandonedBuildings > 5
            ? "medium"
            : "low",
    });
  }
...
function calculateDaysBetween(
  ...
  const startDays = start.year * 360 + (start.month - 1) * 30 + start.day;
  const endDays = end.year * 360 + (end.month - 1) * 30 + end.day;
  return endDays - startDays;
}

const PETITION_TEMPLATES: Array<{
  category: PetitionCategory;
  trigger: (stats: Stats, taxRate: number) => boolean;
  generate: (
    stats: Stats,
    taxRate: number,
  ) => Omit<Petition, "id" | "createdAt" | "signatures">;
}> = [
  {
    category: "taxes",
    trigger: (_, taxRate) => taxRate > 12,
    generate: (_, taxRate) => ({
      title: "Lower Our Taxes!",
      description: `Citizens are struggling under the current ${taxRate}% tax rate. Many families are considering leaving the city.`,
      category: "taxes",
      icon: "cash",
      request: "Please reduce the tax rate to help working families.",
      acceptEffect: {
        description: "You promise to review the tax policy.",
        happinessChange: 8,
        moneyChange: 0,
        action: "lower_taxes",
      },
      rejectEffect: {
        description: "Citizens feel ignored by city hall.",
        happinessChange: -10,
      },
      expiresInDays: 14,
      urgency: taxRate > 15 ? "high" : "medium",
    }),
  },
  {
    category: "services",
    trigger: (stats) => stats.safety < 40,
    generate: (stats) => ({
      title: "We Need More Police!",
      description: `Crime rates are alarming. Our safety rating is only ${Math.round(stats.safety)}%. Residents don't feel safe.`,
      category: "services",
      icon: "shield",
      request: "Build more police stations to protect our neighborhoods.",
      acceptEffect: {
        description: "You commit to improving public safety.",
        happinessChange: 5,
        moneyChange: -500,
        action: "build_police",
      },
      rejectEffect: {
        description: "Citizens feel abandoned.",
        happinessChange: -8,
      },
      expiresInDays: 21,
      urgency: stats.safety < 25 ? "high" : "medium",
    }),
  },
  {
    category: "services",
    trigger: (stats) => stats.health < 45,
    generate: (stats) => ({
      title: "Healthcare Crisis!",
      description: `Health services are inadequate. Our health rating is only ${Math.round(stats.health)}%. People are getting sick.`,
      category: "services",
      icon: "hospital",
      request: "We desperately need a new hospital.",
      acceptEffect: {
        description: "You promise to improve healthcare access.",
        happinessChange: 6,
        moneyChange: -1000,
        action: "build_hospital",
      },
      rejectEffect: {
        description: "Citizens are disappointed in city leadership.",
        happinessChange: -7,
      },
      expiresInDays: 21,
      urgency: stats.health < 30 ? "high" : "medium",
    }),
  },
  {
    category: "services",
    trigger: (stats) => stats.education < 45,
    generate: (stats) => ({
      title: "Our Children Deserve Better!",
      description: `Education levels are falling. Our education rating is only ${Math.round(stats.education)}%. The youth need proper schools.`,
      category: "services",
      icon: "education",
      request: "Build a new school in our neighborhood.",
      acceptEffect: {
        description: "You commit to investing in education.",
        happinessChange: 6,
        moneyChange: -400,
        action: "build_school",
      },
      rejectEffect: {
        description: "Parents are upset with the lack of investment.",
        happinessChange: -6,
      },
      expiresInDays: 21,
      urgency: stats.education < 30 ? "high" : "medium",
    }),
  },
  {
    category: "environment",
    trigger: (stats) => stats.environment < 40,
    generate: (stats) => ({
      title: "Clean Up Our City!",
      description: `Pollution is ruining our quality of life. The environment rating is only ${Math.round(stats.environment)}%.`,
      category: "environment",
      icon: "environment",
      request: "We need more parks and green spaces.",
      acceptEffect: {
        description: "You promise to improve environmental conditions.",
        happinessChange: 5,
        moneyChange: -150,
        action: "build_park",
      },
      rejectEffect: {
        description: "Environmental activists are outraged.",
        happinessChange: -6,
      },
      expiresInDays: 14,
      urgency: stats.environment < 25 ? "high" : "medium",
    }),
  },
  {
    category: "general",
    trigger: (stats) => stats.happiness < 50,
    generate: (stats) => ({
      title: "City Morale is Low!",
      description: `Citizens are unhappy. The overall happiness rating is only ${Math.round(stats.happiness)}%. Something needs to change.`,
      category: "general",
      icon: "planning",
      request: "Please take action to improve quality of life.",
      acceptEffect: {
        description: "You promise to address citizen concerns.",
        happinessChange: 10,
        moneyChange: -200,
      },
      rejectEffect: {
        description: "Citizens feel their voices don't matter.",
        happinessChange: -12,
      },
      expiresInDays: 10,
      urgency: stats.happiness < 35 ? "high" : "medium",
    }),
  },
  {
    category: "zoning",
    trigger: (stats) =>
      stats.jobs < stats.population * 0.7 && stats.population > 200,
    generate: (stats) => ({
      title: "We Need Jobs!",
      description: `Unemployment is high. We only have ${stats.jobs} jobs for ${stats.population} residents. Zone more commercial and industrial areas.`,
      category: "zoning",
      icon: "jobs",
      request: "Create more jobs by expanding business districts.",
      acceptEffect: {
        description: "You commit to economic development.",
        happinessChange: 5,
        moneyChange: 0,
      },
      rejectEffect: {
        description: "Unemployed citizens are frustrated.",
        happinessChange: -8,
      },
      expiresInDays: 21,
      urgency: stats.jobs < stats.population * 0.5 ? "high" : "medium",
    }),
  },
];
...
function generatePetition(
  ...
  const eligibleTemplates = PETITION_TEMPLATES.filter((t) =>
    t.trigger(stats, taxRate),
  );
  ...
  const base = template.generate(stats, taxRate);
  const signatureBase = Math.floor(stats.population * 0.05);
  const signatures = Math.max(
    10,
    signatureBase + Math.floor(Math.random() * signatureBase),
  );

  return {
    ...base,
    id: `petition-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
interface EventTemplate {
  ...
  trigger: (stats: Stats, population: number) => boolean;
  weight: number;
  generate: (stats: Stats) => Omit<SpecialEvent, "id" | "createdAt">;
}

const SPECIAL_EVENT_TEMPLATES: EventTemplate[] = [
  {
    type: "positive",
    category: "economic",
    trigger: (stats) => stats.population > 500 && stats.happiness > 60,
    weight: 2,
    generate: () => ({
      title: "New Business Opportunity!",
      description:
        "A major corporation is interested in opening headquarters in your city. This could bring jobs and prestige.",
      type: "positive",
      category: "economic",
      icon: "cash",
      choices: [
        {
          ...
          description: "Reduce their taxes to seal the deal",
          effects: {
            moneyChange: -5000,
            happinessChange: 8,
            populationChange: 50,
        ...
        },
        {
          ...
          description: "Welcome them without special treatment",
          effects: { happinessChange: 3, populationChange: 20 },
        },
        {
          label: "Decline Politely",
          description: "We don't need outside corporations",
          effects: { happinessChange: -2 },
        },
      ],
      expiresInDays: 7,
    }),
  },
  {
    ...
    trigger: (stats) => stats.population > 200,
    weight: 3,
    generate: () => ({
      title: "City Festival Proposal",
      description:
        "Citizens want to organize a festival to celebrate our city. It would boost morale but cost money.",
      type: "positive",
      category: "celebration",
      icon: "planning",
      choices: [
        {
          ...
          description: "Spare no expense!",
          effects: { moneyChange: -3000, happinessChange: 15 },
        },
        {
          label: "Modest Celebration",
          description: "A smaller but affordable event",
          effects: { moneyChange: -500, happinessChange: 8 },
        },
        {
          label: "Not This Year",
          description: "The budget is too tight",
  ...
  },
  {
    type: "positive",
    category: "social",
    trigger: (stats) => stats.education > 70 && stats.population > 300,
    weight: 1,
    generate: () => ({
      title: "Research Grant Available!",
      description:
        "A federal research grant is available for cities with strong education systems. This could fund a new research facility.",
      type: "positive",
      category: "social",
      icon: "education",
      choices: [
        {
          ...
          description: "Submit an application ($500 fee)",
          effects: { moneyChange: 8000, educationChange: 5 },
        },
        {
  ...
  },
  {
    ...
    trigger: (stats) => stats.population > 400,
    weight: 2,
    generate: () => ({
      title: "Water Main Break!",
      description:
        "A major water main has burst, affecting water service to parts of the city. Emergency repairs are needed.",
      type: "negative",
      category: "infrastructure",
      icon: "water",
      choices: [
        {
          ...
          description: "Fix it immediately at premium cost",
          effects: { moneyChange: -2000, happinessChange: -2 },
        },
        {
          label: "Standard Repairs",
          description: "Schedule regular repairs (takes longer)",
          effects: { moneyChange: -800, happinessChange: -8, healthChange: -5 },
        },
      ],
      expiresInDays: 3,
    }),
  },
  {
    type: "negative",
    category: "social",
    trigger: (stats) => stats.happiness < 50 && stats.population > 300,
    weight: 2,
    generate: () => ({
      title: "Worker Strike!",
      description:
        "City workers are threatening to strike over working conditions. Production may slow.",
      type: "negative",
      category: "social",
      icon: "jobs",
      choices: [
        {
          ...
          description: "Agree to better conditions",
          effects: { moneyChange: -3000, happinessChange: 10 },
        },
        {
          label: "Negotiate",
          description: "Try to find a compromise",
          effects: { moneyChange: -1000, happinessChange: 2 },
        },
        {
          label: "Refuse",
          description: "Stand firm against demands",
  ...
  },
  {
    type: "negative",
    category: "environment",
    trigger: (stats) => stats.environment < 40 && stats.population > 200,
    weight: 2,
    generate: () => ({
      title: "Pollution Alert!",
      description:
        "Air quality has reached dangerous levels. Health officials recommend action.",
      type: "negative",
      category: "environment",
      icon: "environment",
      choices: [
        {
          ...
          description: "Implement costly cleanup measures",
          effects: {
            moneyChange: -4000,
        ...
        },
        {
          ...
          description: "Warn citizens but take no action",
          effects: { happinessChange: -5, healthChange: -5 },
        },
      ],
      expiresInDays: 7,
    }),
  },
  {
    type: "neutral",
    category: "economic",
    trigger: (stats) => stats.population > 1000,
    weight: 1,
    generate: () => ({
      title: "Trade Agreement Opportunity",
      description:
        "A neighboring city proposes a trade agreement. It could help or hurt local businesses.",
      type: "neutral",
      category: "economic",
      icon: "cash",
      choices: [
        {
          ...
          description: "Open trade routes",
          effects: { moneyChange: 2000, happinessChange: -3 },
        },
        {
          label: "Counter-Propose",
          description: "Negotiate better terms",
          effects: { moneyChange: 500 },
        },
        {
          label: "Decline",
          description: "Protect local businesses",
  ...
  },
  {
    type: "positive",
    category: "social",
    trigger: (stats) => stats.safety > 70 && stats.population > 500,
    weight: 1,
    generate: () => ({
      title: "Film Production Interest",
      description:
        "A movie studio wants to film in your city, praising its safety and charm.",
      type: "positive",
      category: "social",
      icon: "planning",
      choices: [
        {
          label: "Welcome Them",
          description: "Allow filming with city cooperation",
          effects: { moneyChange: 5000, happinessChange: 5 },
        },
        {
          label: "Charge Premium",
          description: "Demand higher fees for access",
          effects: { moneyChange: 8000, happinessChange: -2 },
        },
        {
          label: "Too Disruptive",
          description: "Decline to avoid traffic issues",
          effects: {},
        },
      ],
      expiresInDays: 7,
...
function generateSpecialEvent(
  ...
  const eligibleTemplates = SPECIAL_EVENT_TEMPLATES.filter((t) =>
    t.trigger(stats, stats.population),
  );
  ...
  if (!selectedTemplate) {
    selectedTemplate = eligibleTemplates[0];
  }

  const base = selectedTemplate.generate(stats);
...
interface AchievementDefinition {
  ...
  icon: string;
  check: (stats: Stats, grid: Tile[][], year: number) => boolean;
}
...
const ACHIEVEMENT_DEFINITIONS: AchievementDefinition[] = [
  {
    ...
    category: "population",
    icon: "planning",
    check: (stats) => stats.population >= 100,
  },
  {
    id: "pop_500",
    ...
    category: "population",
    icon: "planning",
    check: (stats) => stats.population >= 500,
  },
  {
    id: "pop_1000",
    name: "Town Status",
    ...
    category: "population",
    icon: "planning",
    check: (stats) => stats.population >= 1000,
  },
  {
    id: "pop_5000",
    name: "Small City",
    ...
    category: "population",
    icon: "planning",
    check: (stats) => stats.population >= 5000,
  },
  {
    id: "pop_10000",
    name: "City Status",
    ...
    category: "population",
    icon: "planning",
    check: (stats) => stats.population >= 10000,
  },
  {
    id: "pop_25000",
    name: "Major City",
    description: "Reach 25,000 citizens",
    category: "population",
    icon: "planning",
    check: (stats) => stats.population >= 25000,
  },
  {
    id: "pop_50000",
    name: "Metropolis",
    description: "Reach 50,000 citizens",
    category: "population",
    icon: "planning",
    check: (stats) => stats.population >= 50000,
  },
  {
    id: "money_10000",
    name: "Stable Finances",
    description: "Accumulate $10,000 in treasury",
    category: "economy",
    icon: "cash",
  ...
  },
  {
    ...
    description: "Accumulate $50,000 in treasury",
    category: "economy",
    icon: "cash",
  ...
  },
  {
    ...
    description: "Accumulate $500,000 in treasury",
    category: "economy",
    icon: "cash",
    check: (stats) => stats.money >= 500000,
  },
  {
    id: "happiness_80",
    name: "Happy Citizens",
    ...
    category: "services",
    icon: "planning",
    check: (stats) => stats.happiness >= 80,
  },
  {
    id: "happiness_95",
    name: "Utopia",
    description: "Achieve 95% happiness rating",
    category: "services",
    icon: "planning",
    check: (stats) => stats.happiness >= 95,
  },
  {
    id: "education_90",
    name: "Center of Learning",
    description: "Achieve 90% education rating",
    category: "services",
    icon: "education",
    check: (stats) => stats.education >= 90,
  },
  {
    id: "safety_90",
    name: "Safe Haven",
    description: "Achieve 90% safety rating",
    category: "services",
    icon: "shield",
    check: (stats) => stats.safety >= 90,
  },
  {
    id: "health_90",
    name: "Healthy Living",
    description: "Achieve 90% health rating",
    category: "services",
    icon: "hospital",
  ...
  },
  {
    id: "environment_80",
    name: "Green City",
    description: "Achieve 80% environment rating",
    category: "services",
    icon: "environment",
    check: (stats) => stats.environment >= 80,
  },
  {
    id: "year_5",
    name: "Five Year Plan",
    description: "Lead the city for 5 years",
    category: "longevity",
    icon: "planning",
    check: (_, __, year) => year >= 2029,
  },
  {
    id: "year_10",
    name: "Decade of Leadership",
    description: "Lead the city for 10 years",
    category: "longevity",
    icon: "planning",
    check: (_, __, year) => year >= 2034,
  },
  {
    id: "year_25",
    name: "Silver Jubilee",
    description: "Lead the city for 25 years",
    category: "longevity",
    icon: "planning",
    check: (_, __, year) => year >= 2049,
  },
  {
    id: "first_stadium",
    name: "Sports Fan",
    description: "Build your first stadium",
    category: "buildings",
    icon: "planning",
    check: (_, grid) =>
      grid.some((row) => row.some((t) => t.building.type === "stadium")),
  },
  {
    id: "first_airport",
    name: "Ready for Takeoff",
    description: "Build your first airport",
    category: "buildings",
    icon: "planning",
    check: (_, grid) =>
      grid.some((row) => row.some((t) => t.building.type === "airport")),
  },
  {
    id: "first_university",
    name: "Higher Education",
    description: "Build your first university",
    category: "buildings",
    icon: "education",
    check: (_, grid) =>
      grid.some((row) => row.some((t) => t.building.type === "university")),
  },
  {
    id: "space_program",
    name: "To the Stars",
    description: "Build a space program facility",
    category: "buildings",
    icon: "planning",
    check: (_, grid) =>
      grid.some((row) => row.some((t) => t.building.type === "space_program")),
  },
];
...
function checkAchievements(
  ...
  for (const def of ACHIEVEMENT_DEFINITIONS) {
    if (!unlockedIds.has(def.id) && def.check(stats, grid, year)) {
...
export function simulateTick(state: GameState): GameState {
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      const isCompletedServiceBuilding =
        originalTile.zone === "none" &&
      ...
      if (
        ...
        const isUtilityBuilding =
          tile.building.type === "power_plant" ||
          tile.building.type === "water_tower";
        const canConstruct =
          isUtilityBuilding || (tile.building.powered && tile.building.watered);

        if (canConstruct) {
          const constructionSpeed = getConstructionSpeed(tile.building.type);
          tile.building.constructionProgress = Math.min(
            100,
            tile.building.constructionProgress + constructionSpeed,
          );
        }
      }
      ...
      if (tile.zone !== "none" && tile.building.type === "grass") {
        ...
        const demandFactor = Math.max(
          0,
          Math.min(1, (zoneDemandForSpawn + 30) / 80),
        );
        ...
        newGrid[y][x].building = evolveBuilding(
          newGrid,
          x,
          y,
          services,
      ...
      tile.pollution = Math.max(
        0,
        tile.pollution * 0.95 + (buildingStats?.pollution || 0),
      );
      ...
      if (
        ...
        if (adjacentFireCount > 0) {
          ...
          const spreadChance =
            baseSpreadChance * (1 - coverageReduction * 0.95); // Fire coverage can reduce spread by up to 95%
  ...
  if (prevDemand) {
    ...
    newStats.demand.residential =
      prevDemand.residential +
      (newStats.demand.residential - prevDemand.residential) * smoothingFactor;
    newStats.demand.commercial =
      prevDemand.commercial +
      (newStats.demand.commercial - prevDemand.commercial) * smoothingFactor;
    newStats.demand.industrial =
      prevDemand.industrial +
      (newStats.demand.industrial - prevDemand.industrial) * smoothingFactor;
  }
  ...
  if (isDayChange) {
    const currentDate = { year: newYear, month: newMonth, day: newDay };

    if (activePetition) {
      const petitionAge = calculateDaysBetween(
        activePetition.createdAt,
        currentDate,
      );
      const hasExpired = petitionAge > activePetition.expiresInDays;
    ...
    const PETITION_CHANCE_PER_DAY = 0.3;
    const shouldGeneratePetition =
      !activePetition &&
      newStats.population > 100 &&
      Math.random() < PETITION_CHANCE_PER_DAY;

    if (shouldGeneratePetition) {
      activePetition = generatePetition(newStats, state.taxRate, currentDate);
    }

    if (activeEvent) {
      const eventAge = calculateDaysBetween(activeEvent.createdAt, currentDate);
      const hasExpired = eventAge > activeEvent.expiresInDays;
    ...
    const shouldGenerateEvent =
      !activeEvent &&
      !activePetition &&
      newStats.population > 150 &&
      Math.random() < EVENT_CHANCE_PER_DAY;

    if (shouldGenerateEvent) {
      activeEvent = generateSpecialEvent(newStats, currentDate);
    }
  }
  ...
  const newlyUnlocked = achievements.filter(
    (a) => !(state.achievements || []).some((ea) => ea.id === a.id),
  );
...
const BUILDING_SIZES: Partial<
  Record<BuildingType, { width: number; height: number }>
> = {
...
export function getBuildingSize(buildingType: BuildingType): {
  width: number;
  height: number;
} {
...
function isMergeableZoneTile(
  ...
  if (
    allowBuildingConsolidation &&
    CONSOLIDATABLE_BUILDINGS[zone]?.has(tile.building.type)
  ) {
    return true;
  }
...
function footprintAvailable(
  ...
  if (
    originX < 0 ||
    originY < 0 ||
    originX + width > gridSize ||
    originY + height > gridSize
  ) {
    return false;
  }
...
export function placeBuilding(
  ...
  if (
    ...
    buildingType !== "road" &&
    buildingType !== "rail" &&
    (tile.building.type === "road" || tile.building.type === "bridge")
  ) {
    return state;
  }
  if (
...
function findConnectedBridgeTiles(
  ...
  while (cx >= 0 && cx < gridSize && cy >= 0 && cy < gridSize) {
    const t = grid[cy][cx];
    if (
      t.building.type === "bridge" &&
      t.building.bridgeOrientation === orientation
    ) {
...
export function bulldozeTile(
  ...
  if (tile.building.type === "road") {
    const adjacentBridgeTiles = findAdjacentBridgeTiles(
      newGrid,
      state.gridSize,
      x,
      y,
...
export function placeWaterTerraform(
  state: GameState,
  x: number,
  y: number,
): GameState {
...
export function placeLandTerraform(
  state: GameState,
  x: number,
  y: number,
): GameState {
...
export function generateRandomAdvancedCity(
  ...
  const placeZonedBuilding = (
    ...
    if (
      ...
      const stats = BUILDING_STATS[buildingType];
      if (stats) {
        tile.building.population = Math.floor(
          stats.maxPop * tile.building.level * 0.7,
        );
        tile.building.jobs = Math.floor(
          stats.maxJobs * tile.building.level * 0.7,
        );
      }
    }
  };
  ...
  const placeMultiTileBuilding = (
    ...
    if (!isRegionClear(x, y, buildingSize.width, buildingSize.height))
      return false;
    if (x + buildingSize.width > size || y + buildingSize.height > size)
      return false;
    ...
    for (let dy = 0; dy < buildingSize.height; dy++) {
      for (let dx = 0; dx < buildingSize.width; dx++) {
        ...
        if (dx === 0 && dy === 0) {
          ...
          if (stats) {
            tile.building.population = Math.floor(
              stats.maxPop * tile.building.level * 0.8,
            );
            tile.building.jobs = Math.floor(
              stats.maxJobs * tile.building.level * 0.8,
            );
          }
        } else {
  ...
  for (
    let roadY = centerY - cityRadius;
    roadY <= centerY + cityRadius;
    roadY += roadSpacing
  ) {
  ...
  // Main vertical roads
  for (
    let roadX = centerX - cityRadius;
    roadX <= centerX + cityRadius;
    roadX += roadSpacing
  ) {
  ...
  for (let i = 0; i < 25 + Math.floor(Math.random() * 15); i++) {
    const parkType =
      parkBuildings[Math.floor(Math.random() * parkBuildings.length)];
    let attempts = 0;
    while (attempts < 100) {
      const x =
        centerX - cityRadius + Math.floor(Math.random() * cityRadius * 2);
  ...
  for (let y = 0; y < size; y++) {
    for (let x = 0; x < size; x++) {
      ...
      if (normalizedDist < 0.3) {
        ...
        if (rand < 0.6) {
          zone = "commercial";
          const commercialTypes: BuildingType[] = [
            "shop_small",
            "shop_medium",
            "office_low",
            "office_high",
          ...
          zone = "residential";
          const residentialTypes: BuildingType[] = [
            "apartment_low",
            "apartment_high",
          ];
          buildingType =
            residentialTypes[
              Math.floor(Math.random() * residentialTypes.length)
            ];
        }
        ...
        if (rand < 0.5) {
          ...
          zone = "industrial";
          const industrialTypes: BuildingType[] = [
            "factory_small",
            "factory_medium",
            "warehouse",
          ];
  ...
  for (
    let y = centerY - Math.floor(cityRadius * 0.6);
    y <= centerY + Math.floor(cityRadius * 0.6);
    y++
  ) {
    for (
      let x = centerX - Math.floor(cityRadius * 0.6);
      x <= centerX + Math.floor(cityRadius * 0.6);
      x++
    ) {
      const tile = grid[y]?.[x];
      if (tile && tile.building.type !== "water") {
        ...
        const onMainRoad =
          x % roadSpacing === centerX % roadSpacing ||
          y % roadSpacing === centerY % roadSpacing;
  ...
  for (
    let y = centerY - cityRadius;
    y <= centerY + cityRadius;
    y += subwayStationSpacing
  ) {
    for (
      let x = centerX - cityRadius;
      x <= centerX + cityRadius;
      x += subwayStationSpacing
    ) {
      const tile = grid[y]?.[x];
...
export function getDevelopmentBlockers(
  ...
  if (!roadAccess) {
    blockers.push({
      ...
      details:
        "Tile must be within 8 tiles of a road (through same-zone tiles)",
    });
  }
  ...
  if (candidateSize.width > 1 || candidateSize.height > 1) {
    ...
    if (
      ...
      if (
        x + candidateSize.width > state.gridSize ||
        y + candidateSize.height > state.gridSize
      ) {
      ...
      for (
        let dy = 0;
        dy < candidateSize.height && footprintBlockers.length < 3;
        dy++
      ) {
        for (
          let dx = 0;
          dx < candidateSize.width && footprintBlockers.length < 3;
          dx++
        ) {
          const checkTile = state.grid[y + dy]?.[x + dx];
          if (!checkTile) {
            footprintBlockers.push(
              `Tile (${x + dx}, ${y + dy}) is out of bounds`,
            );
          } else if (checkTile.zone !== tile.zone) {
            footprintBlockers.push(
              `Tile (${x + dx}, ${y + dy}) has different zone: ${checkTile.zone}`,
            );
          } else if (
            checkTile.building.type !== "grass" &&
            checkTile.building.type !== "tree"
            ...
            footprintBlockers.push(
              `Tile (${x + dx}, ${y + dy}) has ${checkTile.building.type}`,
            );
          }
        }
      }
...
export function expandGrid(
  ...
  const getClosestEdgeType = (
    ...
    if (
      (oldX < 0 || oldX >= currentSize) &&
      (oldY < 0 || oldY >= currentSize)
    ) {
  ...
  const getDistanceFromOriginalBoundary = (
    newX: number,
    newY: number,
  ): number => {
    const oldX = newX - expansion;
    ...
    if (oldX >= currentSize)
      distToOriginal = Math.max(distToOriginal, oldX - currentSize + 1);
    if (oldY >= currentSize)
      distToOriginal = Math.max(distToOriginal, oldY - currentSize + 1);
  ...
  const getEdgeWaterInfo = (
    newX: number,
    newY: number,
    sampleRadius: number = 10,
  ): { density: number; isSea: boolean } => {
    const oldX = newX - expansion;
    const oldY = newY - expansion;
  ...
  for (let y = 0; y < newSize; y++) {
    ...
    for (let x = 0; x < newSize; x++) {
      ...
      const wasInOldGrid =
        oldX >= 0 && oldY >= 0 && oldX < currentSize && oldY < currentSize;
      ...
      if (wasInOldGrid) {
        ...
        if (isSea) {
          ...
          const distFromNewEdge = Math.min(
            x,
            y,
            newSize - 1 - x,
            newSize - 1 - y,
          ...
          const maxExpansionDist =
            expansion * (0.5 + coastNoise * 0.3 + edgeWaterDensity * 0.4);

          // Probability = 1 at boundary, tapers to 0 at maxExpansionDist
          const taperRatio = distFromBoundary / maxExpansionDist;
          const waterProb =
            Math.max(0, 1 - Math.pow(taperRatio, 1.3)) * edgeWaterDensity;
  ...
  for (let iteration = 0; iteration < 4; iteration++) {
    ...
    for (const change of changes) {
      if (change.toWater) {
        ...
        grid[change.y][change.x].building = createBuilding(
          treeNoise < 0.15 ? "tree" : "grass",
        );
      }
    }
  }
  ...
  for (let y = 0; y < newSize; y++) {
    for (let x = 0; x < newSize; x++) {
      ...
      if (isWater && neighbors <= 1) {
        ...
        grid[y][x].building = createBuilding(
          treeNoise < 0.15 ? "tree" : "grass",
        );
  ...
  const lakeNoise = (lx: number, ly: number) =>
    perlinNoise(lx, ly, expansionSeed + 3000, 3);
  ...
  const numBigLakes = Math.min(
    bigLakeCenters.length,
  ...
  const pondNoise = (px: number, py: number) =>
    perlinNoise(px, py, expansionSeed + 4000, 2);
  ...
  const numSmallLakes = Math.min(
    smallLakeCenters.length,
  ...
  if (Math.random() < riverChance) {
    ...
    for (let r = 0; r < numRivers; r++) {
      ...
      for (let attempts = 0; attempts < 50 && !foundStart; attempts++) {
        const testX =
          minDistFromEdge +
          Math.floor(Math.random() * (newSize - 2 * minDistFromEdge));
        const testY =
          minDistFromEdge +
          Math.floor(Math.random() * (newSize - 2 * minDistFromEdge));

        if (
          isExpansionTile(testX, testY) &&
          grid[testY][testX].building.type === "water"
        ) {
      ...
      for (let step = 0; step < riverLength; step++) {
        ...
        const riverNoise = perlinNoise(
          curX * 0.2,
          curY * 0.2,
          expansionSeed + 5000 + r * 100,
          2,
        );
        const angle =
          Math.atan2(endY - curY, endX - curX) +
          (riverNoise - 0.5) * Math.PI * 0.8;
