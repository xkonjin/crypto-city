...
function saveCityToIndex(state: GameState, roomCode?: string): void {
  ...
  try {
    ...
    const existingIndex = cities.findIndex(
      (c) => c.id === cityMeta.id || (roomCode && c.roomCode === roomCode),
    );
...
function SpriteGallery({
  ...
  const [filteredSheet, setFilteredSheet] = useState<HTMLCanvasElement | null>(
    null,
  );
  ...
  const randomSpriteKeys = useMemo(() => {
    ...
    const validSpriteKeys = spritePack.spriteOrder.filter((spriteKey) => {
      ...
      const hasBuildingMapping = Object.values(
  ...
  const spriteData = useMemo(() => {
    ...
    return randomSpriteKeys
      .map((spriteKey) => {
        const buildingType =
          Object.entries(spritePack.buildingToSprite).find(
    ...
    <div className="relative group">
      ...
      >
        ...
        <div className="flex items-center gap-3 mt-1 text-xs text-white/50">
          ...
          {city.roomCode && (
            <span className="text-blue-400/60">{city.roomCode}</span>
          )}
        </div>
      </button>
      {onDelete && (
