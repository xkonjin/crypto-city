...
type GameContextValue = {
  ...
  setPlaceCallback: (
    callback: ((args: { x: number; y: number; tool: Tool }) => void) | null,
  ) => void;
  finishTrackDrag: (
...
async function saveGameStateAsync(state: GameState): Promise<void> {
  ...
  try {
    ...
    if (compressed.length > 5 * 1024 * 1024) {
      console.error(
        "Compressed game state too large to save:",
        compressed.length,
        "chars",
      );
      return;
    ...
    try {
      ...
      if (
        ...
        (quotaError.code === 22 || quotaError.code === 1014)
      ) {
...
function decompressSavedCity(
  saved: string,
): { state?: GameState; info?: SavedCityInfo } | null {
  // Try to decompress first (new format)
  let jsonString = decompressFromUTF16(saved);
  if (!jsonString) {
...
// PERF: Uses Web Worker for BOTH serialization and compression - no main thread blocking!
async function saveCityStateAsync(
  cityId: string,
  state: GameState,
): Promise<void> {
  if (typeof window === "undefined") return;
...
export function GameProvider({
  ...
  const [state, setState] = useState<GameState>(() =>
    createInitialGameState(DEFAULT_GRID_SIZE, "CryptoCity"),
  );
  ...
  const placeCallbackRef = useRef<
    ((args: { x: number; y: number; tool: Tool }) => void) | null
  >(null);
  const bridgeCallbackRef = useRef<
    | ((args: {
        pathTiles: { x: number; y: number }[];
        trackType: "road" | "rail";
      }) => void)
  ...
  const setPlaceCallback = useCallback(
    (
      callback: ((args: { x: number; y: number; tool: Tool }) => void) | null,
    ) => {
  ...
  const setBridgeCallback = useCallback(
    (
      callback:
        | ((args: {
            pathTiles: { x: number; y: number }[];
  ...
  const acceptPetition = useCallback(() => {
    setState((prev) => {
      if (!prev.activePetition) return prev;

      const petition = prev.activePetition;
      const happinessChange = petition.acceptEffect.happinessChange;
      const moneyChange = petition.acceptEffect.moneyChange;

      return {
        ...prev,
        activePetition: null,
        petitionHistory: [
          ...(prev.petitionHistory || []),
          {
            petition,
            resolution: "accepted",
            resolvedAt: { year: prev.year, month: prev.month, day: prev.day },
          },
        ],
        stats: {
          ...prev.stats,
          happiness: Math.min(
            100,
            Math.max(0, prev.stats.happiness + happinessChange),
          ),
          money: prev.stats.money + moneyChange,
        },
        notifications: [
          {
            id: `petition-accepted-${Date.now()}`,
            title: "Petition Accepted",
            description: petition.acceptEffect.description,
            icon: "planning",
            timestamp: Date.now(),
          },
          ...prev.notifications,
  ...
  const rejectPetition = useCallback(() => {
    setState((prev) => {
      if (!prev.activePetition) return prev;

      const petition = prev.activePetition;
      const happinessChange = petition.rejectEffect.happinessChange;
      ...
      return {
        ...
        notifications: [
          {
            id: `petition-rejected-${Date.now()}`,
            title: "Petition Rejected",
            description: petition.rejectEffect.description,
            icon: "planning",
            timestamp: Date.now(),
          },
          ...prev.notifications,
    ...
    setState((prev) => {
      ...
      if (effects.happinessChange) {
        newStats.happiness = Math.min(
          100,
          Math.max(0, newStats.happiness + effects.happinessChange),
        );
      }
      if (effects.moneyChange) {
        newStats.money += effects.moneyChange;
      }
      if (effects.populationChange) {
        newStats.population = Math.max(
          0,
          newStats.population + effects.populationChange,
        );
      }
      if (effects.safetyChange) {
        newStats.safety = Math.min(
          100,
          Math.max(0, newStats.safety + effects.safetyChange),
        );
      }
      if (effects.healthChange) {
        newStats.health = Math.min(
          100,
          Math.max(0, newStats.health + effects.healthChange),
        );
      }
      if (effects.educationChange) {
        newStats.education = Math.min(
          100,
          Math.max(0, newStats.education + effects.educationChange),
        );
      }
      if (effects.environmentChange) {
        newStats.environment = Math.min(
          100,
          Math.max(0, newStats.environment + effects.environmentChange),
        );
      }
